...
/// Tests that the DUMBPIPE_RELAYS environment variable is wired up to the
/// `--relay` flag: an invalid URL supplied via the env var should cause the
/// same clap parse failure as an invalid `--relay` value.
#[test]
fn dumbpipe_relays_env_var_invalid_url_fails() {
    let output = duct::cmd(dumbpipe_bin(), ["listen"])
        .env("DUMBPIPE_RELAYS", "not-a-url")
        .env_remove("RUST_LOG")
        .stdin_bytes(&b""[..])
        .stderr_capture()
        .unchecked()
    ...
    assert!(
        stderr.contains("DUMBPIPE_RELAYS") || stderr.contains("relay"),
        "unexpected stderr: {stderr}"
    );
}

/// A quick clap-error test: an invalid `--relay` URL should exit nonzero
/// before ever attempting to bind an endpoint.
#[test]
fn relay_flag_rejects_invalid_url() {
    let output = duct::cmd(dumbpipe_bin(), ["listen", "--relay", "not-a-url"])
        .env_remove("RUST_LOG")
        .stdin_bytes(&b""[..])
        .stderr_capture()
        .unchecked()
